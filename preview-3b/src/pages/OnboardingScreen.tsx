...
export function OnboardingScreen({ onComplete }: OnboardingScreenProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-black relative">
      <button
        onClick={onComplete}
        className="absolute top-12 right-6 text-white/50 font-medium text-sm z-10 font-body">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-8 relative overflow-hidden">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            <div className="w-48 h-48 bg-arozen-gold/20 rounded-full flex items-center justify-center mb-10">
              {createElement(SLIDES[currentSlide].icon, {
                className: 'w-24 h-24 text-arozen-gold',
                strokeWidth: 1.5
              })}
            </div>
            <h2 className="text-2xl font-medium text-white mb-4 font-heading">
      ...
      <div className="px-8 pb-12 pt-4">
        <div className="flex justify-center space-x-2 mb-8">
          ...
          <div
            key={idx}
            className={`h-2 rounded-full transition-all duration-300 ${idx === currentSlide ? 'w-6 bg-arozen-gold' : 'w-2 bg-white/20'}`} />
        ...
        <button
          onClick={handleNext}
          className="w-full bg-arozen-gold text-white font-medium py-4 rounded-xl shadow-lg shadow-arozen-gold/20 active:scale-[0.98] transition-transform font-body">

    ...
    <div className="bg-white rounded-2xl p-4 mb-4 relative">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <label className="text-xs font-medium text-gray-700 font-body">
            Intensity
          </label>
          <span className="text-xs text-gray-400 font-body">
            {INTENSITY_NAMES[intensity]} (every {INTENSITY_MINUTES[intensity]}{' '}
            min)
          </span>
        </div>
        <div className="px-1">
          <input
            ...
            max={maxIntensity}
            step={1}
            value={maxIntensity - intensity}
            onChange={(e) =>
            onChange({
              ...block,
              frequency: maxIntensity - Number(e.target.value)
            })
            }
            className="w-full h-2 bg-gray-100 rounded-full appearance-none cursor-pointer intensity-slider"
            style={{
              background: `linear-gradient(to right, #000000 0%, #000000 ${(maxIntensity - intensity) / maxIntensity * 100}%, #F3F4F6 ${(maxIntensity - intensity) / maxIntensity * 100}%, #F3F4F6 100%)`
            }} />
          ...
          <div className="relative mt-3 h-4">
            ...
            <button
              ...
              style={{
                left: `${(maxIntensity - i) / maxIntensity * 100}%`
              }}>

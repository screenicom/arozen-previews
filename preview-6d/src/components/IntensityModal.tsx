...
export const INTENSITY_MINUTES = [1, 3, 5, 10, 20, 40] as const;
// Names run from the shortest interval (most intense) to the longest (least intense)
          ...
          className="absolute bottom-0 inset-x-0 bg-white rounded-t-3xl z-50 px-6 pt-5 pb-8 shadow-2xl">
            ...
            <div className="text-center my-8">
              ...
              className="text-3xl font-medium text-arozen-black font-heading">
                ...
                Every {INTENSITY_MINUTES[localValue]} min (
                {INTENSITY_NAMES[localValue]})
              </motion.p>
              <p className="text-sm text-gray-500 mt-1 font-body leading-relaxed">
                Sprays every {INTENSITY_MINUTES[localValue]} minutes — shorter
                pauses mean stronger, more frequent scenting.
              </p>
            </div>
            ...
            <div className="px-1">
              ...
              step={1}
              value={INTENSITY_LEVELS.length - 1 - localValue}
              onChange={(e) =>
              setLocalValue(
                INTENSITY_LEVELS.length - 1 - Number(e.target.value)
              )
              }
              className="w-full h-2 bg-gray-100 rounded-full appearance-none cursor-pointer intensity-slider"
              style={{
                background: `linear-gradient(to right, #000000 0%, #000000 ${(INTENSITY_LEVELS.length - 1 - localValue) / (INTENSITY_LEVELS.length - 1) * 100}%, #F3F4F6 ${(INTENSITY_LEVELS.length - 1 - localValue) / (INTENSITY_LEVELS.length - 1) * 100}%, #F3F4F6 100%)`
              }} />
            
              {/* Tick labels — reversed: 40m on the left, 1m on the right */}
              ...
              <button
                ...
                style={{
                  left: `${(INTENSITY_LEVELS.length - 1 - i) / (INTENSITY_LEVELS.length - 1) * 100}%`
                }}>

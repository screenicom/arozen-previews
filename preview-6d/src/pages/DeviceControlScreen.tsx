...
import { IntensityModal, INTENSITY_MINUTES } from '../components/IntensityModal';
import {
  CountdownModal,
  COUNTDOWN_LABELS,
  COUNTDOWN_VALUES,
...
export function DeviceControlScreen({
  ...
  return (
    <div className="flex flex-col h-full wall-bg relative overflow-hidden">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-3 pb-6 phone-scroll">
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <PillControl
            icon={WavesIcon}
            label="Intensity"
            value={`Every ${INTENSITY_MINUTES[intensityValue]} min`}
        ...
        <div className="rounded-3xl px-5 pt-5 pb-4 relative overflow-hidden shadow-[0_8px_20px_-8px_rgba(60,40,15,0.18),0_2px_5px_-1px_rgba(60,40,15,0.10)] bg-neutral-100">
          ...
          <motion.img
            ...
            animate={{
              ...
              filter:
              device.isOn && !isOffline ? 'grayscale(0)' : 'grayscale(1)'
            }}
            transition={{
              opacity: {
                duration: 0.7,
        ...
        <button
          ...
          disabled={isOffline}
          className={`w-full bg-neutral-100 rounded-2xl px-4 py-3.5 flex items-center justify-between transition-opacity shadow-[0_8px_20px_-8px_rgba(60,40,15,0.18),0_2px_5px_-1px_rgba(60,40,15,0.10)] ${isOffline ? 'opacity-50' : 'active:scale-[0.99]'} text-left`}>
...
function PillControl({
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`bg-neutral-100 rounded-2xl pl-2 pr-2 py-2 flex items-center gap-2 text-left transition-opacity shadow-[0_8px_20px_-8px_rgba(60,40,15,0.18),0_2px_5px_-1px_rgba(60,40,15,0.10)] ${disabled ? 'opacity-50' : ''} ${mirrored ? 'flex-row-reverse' : ''}`}>

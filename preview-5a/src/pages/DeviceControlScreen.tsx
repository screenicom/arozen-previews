...
export function DeviceControlScreen({
  ...
  return (
    <div className="flex flex-col h-full bg-arozen-grey relative overflow-hidden">
      {/* Header */}
      <div className="bg-white px-5 pt-14 pb-4 flex items-start justify-between shadow-sm shadow-gray-200 z-10">
        <button
          onClick={onBack}
      ...
      {/* Offline notice */}
      {isOffline &&
      <div className="bg-arozen-danger/5 border-y border-arozen-danger/10 px-5 py-2 flex items-center justify-center">
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-3 pb-6 phone-scroll">
        ...
        <div className="bg-[#F3EFE8] rounded-3xl px-5 pt-5 pb-4 relative overflow-hidden">
          {/* Metrics row */}
          ...
          <div className="relative h-[300px] flex items-end justify-center mt-2">
            ...
            <motion.img
              ...
              className="relative h-[280px] w-auto object-contain z-10" />
        ...
        <button
          ...
          disabled={isOffline}
          className={`w-full bg-[#F3EFE8] rounded-2xl px-4 py-3.5 flex items-center justify-between transition-opacity ${isOffline ? 'opacity-50' : 'active:scale-[0.99]'} text-left`}>
...
function PillControl({
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`bg-[#F3EFE8] rounded-full pl-2 pr-3 py-2 flex items-center gap-2 text-left transition-opacity ${disabled ? 'opacity-50' : ''}`}>
      
      <div className="w-9 h-9 rounded-full bg-white flex items-center justify-center flex-shrink-0">
        <Icon className="w-4 h-4 text-gray-700" strokeWidth={1.5} />
      </div>
      <div className="leading-tight min-w-0 flex-1">
        <p className="text-[10px] text-gray-500 font-body truncate">{label}</p>
        <p className="text-[14px] font-medium text-gray-900 font-heading truncate">
          {value}
        </p>
      </div>
      <ChevronDownIcon
  ...
  return (
    <div
      ...
      <div
        ...
        <p
          className={`text-[11px] uppercase tracking-[0.15em] font-medium font-body ${muted ? 'text-gray-400' : 'text-gray-500'}`}>
          
          {label}
        </p>
        <p
          className={`text-[20px] font-semibold font-heading mt-0.5 ${muted ? 'text-gray-400' : 'text-gray-900'}`}>
